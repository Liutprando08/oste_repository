...
def CATEGORIES():
    ...
    addDir(
        "Top Albums", "http://musicmp3.ru/genres.html", 12, art + "topalbums.jpg", ""
    )
    addDir(
        "New Albums",
        "http://musicmp3.ru/new_albums.html",
        12,
        art + "newalbums.jpg",
        "",
    )
    ...
    addDirAudio(
        "Instant Mix Favourite Songs (Shuffle and Play)",
        "url",
        99,
        art + "mixfavouritesongs.jpg",
        "",
        "",
        "",
        "",
    ...
    addDirAudio(
        "Instant Mix Favourite Albums (Shuffle and Play)",
        "url",
        89,
        art + "mixfavouritealbums.jpg",
        "",
        "",
        "",
        "",
    ...
    addDirAudio(
        "Clear Playlist", "url", 100, art + "clearplaylist.jpg", "", "", "", "", ""
    )
...
def charts():
    addDir(
        "BillBoard Hot 20 Singles",
        "http://www.officialcharts.com/charts/billboard-hot-100-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    addDir(
        "BillBoard 20 Albums",
        "http://www.officialcharts.com/charts/billboard-200/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    addDir(
        "Top 100 Streaming Singles",
    ...
    addDir(
        "Top 100 Compilation Albums",
        "http://www.officialcharts.com/charts/official-compilations-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    ...
    addDir(
        ...
        "http://www.officialcharts.com/charts/dance-singles-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    addDir(
        "Top 40 Dance Albums",
        "http://www.officialcharts.com/charts/dance-albums-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    ...
    addDir(
        "Top 40 R&B Albums",
        "http://www.officialcharts.com/charts/r-and-b-albums-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    addDir(
        "Top 40 Rock/Metal Singles",
    ...
    addDir(
        "Top 50 Independent Singles",
        "http://www.officialcharts.com/charts/independent-singles-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    addDir(
        "Top 50 Independent Albums",
    ...
    addDir(
        "Top 20 Country Compilation Albums",
        "http://www.officialcharts.com/charts/country-compilations-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    addDir(
        "Top 50 Classical Compilation Albums",
        "http://www.officialcharts.com/charts/classical-compilation-albums-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    addDir(
        "Top 30 Jazz & Blues Albums",
        "http://www.officialcharts.com/charts/jazz-and-blues-albums-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
    addDir(
        "Top 20 Christian & Gospel Albums",
        "http://www.officialcharts.com/charts/christian-and-gospel-albums-chart/",
        102,
        artbillboard + "billboardcharts.jpg",
        "",
    )
...
def chart_lists(name, url):  # 102
    ...
    if "officialcharts.com" in url:
        ...
        for _list in all_list:
            ...
            if "singles" in name.lower():
                if (
                    ...
                    addDir(
                        f"{artist} - {title}", f"{artist} - {title}", 28, iconimage, ""
                    )
        ...
        link = link.replace("\n", "").replace("\t", "")
        match = re.compile(
            '<span class="this-week">(.+?)</span> <span class="last-week">(.+?)</span></div><div class="row-image"(.+?)<div class="row-title"><h2>(.+?)</h2><h3><a href="(.+?)" data-tracklabel="Artist Name">(.+?)</a>'
        ).findall(link)
...
def artists(url):
    ...
    addDir(
        "All Artists",
        "http://musicmp3.ru/main_artists.html?type=artist&page=1",
        31,
        art + "allartists.jpg",
        "",
    )
    sub_dir = re.compile(
        '<li class="menu_sub__item"><a class="menu_sub__link" href="(.+?)">(.+?)</a></li>'
    ).findall(link)
    for url1, title in sub_dir:
        iconimage = "http://musicmp3.ru/i" + url1.replace(".html", ".jpg").replace(
            "artists", "genres"
        ).replace("tracks", "track")
        if title != "Other":
            addDir(
                title.replace("&amp;", "&"),
                "https://musicmp3.ru" + url1,
                41,
                artgenre
...
def all_artists(name, url):
    ...
    all_artists = re.compile(
        '<li class="small_list__item"><a class="small_list__link" href="(.+?)">(.+?)</a></li>'
    ).findall(link)
...
def genres(name, url):
    ...
    if name == "Top Albums":
        addDir(
            "Top Albums",
            "http://musicmp3.ru/main_albums.html?gnr_id=&sort=top&type=album&page=1",
            15,
...
def all_genres(name, url):
    nxtpgnum = (
        int(
            url.replace(
                "http://musicmp3.ru/main_albums.html?gnr_id=2&sort=top&type=album&page=",
                "",
            )
        )
        + 1
...
def genre_sub_dir(name, url, icon):
    ...
    addDir(
        ...
        15,
        artgenre
        + name.replace("and", "&").replace(" ", "").replace("&amp;", "_").lower()
        + "/"
        + "top"
        + name.replace("and", "_").replace(" ", "").replace("&amp;", "_").lower()
        + ".jpg",
        "",
    )
    sub_dir = re.compile(
...
def genre_sub_dir2(name, url, icon):
    ...
    addDir(
        ...
        url,
        15,
        os.path.join(artgenre, "alltopalbums.jpg"),
        "",
    )
    sub_dir = re.compile(
        '<li class="menu_sub__item"><a class="menu_sub__link" href="(.+?)">(.+?)</a></li>'
...
def compilations_menu():
    addDir(
        "Best Compilations",
        "http://www.goldenmp3.ru/albums_showcase.html?section=compilations&type=albums&page=",
        401,
        art + "topalbums.jpg",
        "1",
    )
    ...
    addDir(
        "Events",
        "http://www.goldenmp3.ru/compilations/events/albums",
        401,
        art + "newalbums.jpg",
        "n",
    )
...
def compilations_list(name, url, iconimage, page):
    link = open_url(url)
    match = re.compile(
        '<a href="(.+?)"><img alt="(.+?)" src="(.+?)"/></a><a class="(.+?)" href="(.+?)">(.+?)</a><span class="(.+?)">(.+?)</span><span class="f_year">(.+?)</span><span class="ga_price">(.+?)</span>'
    ).findall(link)
...
def search_artists(query):
    url = (
        "https://musicmp3.ru/search.html?text=%s&all=artists"
        % urllib.parse.quote_plus(query)
    )
    link = GET_url(url)  # .decode('utf-8')
    all_artists = re.compile(
        '<a class="artist_preview__title" href="(.+?)">(.+?)</a>'
    ).findall(link)
    for url1, title in all_artists:
        icon_path = os.path.join(ARTIST_ART, title + ".jpg")
        if os.path.exists(icon_path):
...
def search_albums(query):
    url = (
        "https://musicmp3.ru/search.html?text=%s&all=albums"
        % urllib.parse.quote_plus(query.replace(" - ", " ").replace("-", " "))
    )
    link = GET_url(url)  # .decode('utf-8')
    link = link.replace(
    ...
    all_albums = re.compile(
        '<a class="album_report__link" href="(.+?)"><img class="album_report__image" src="(.+?)"/><span class="album_report__name">(.+?)</span></a>(.+?)album_report__artist" href="(.+?)">(.+?)</a>, <span class="album_report__date">(.+?)</span>'
    ).findall(link)
...
def search_songs(query):
    playlist = []
    url = "https://musicmp3.ru/search.html?text=%s&all=songs" % urllib.parse.quote_plus(
        query.replace(" - ", " ")
        .replace("-", " ")
        .replace(" FT ", " ")
        .replace(" FEATURING ", " ")
...
def album_list(name, url):
    ...
    try:
        ...
        pass
    all_albums = re.compile(
        '<a class="album_report__link" href="(.+?)"><img alt="(.+?)" class="album_report__image" src="(.+?)"/><span class="album_report__name">(.+?)</span>(.+?)"album_report__artist" href="(.+?)">(.+?)</a>, <span class="album_report__date">(.+?)</span>'
    ).findall(link)
...
def albums(name, url):
    ...
    try:
        ...
        pass
    all_albums = re.compile(
        '<div class="album_report"><h5 class="album_report__heading"><a class="album_report__link" href="(.+?)"><img alt="(.+?)" class="album_report__image" src="(.+?)"/><span class="album_report__name">(.+?)</span></a></h5><div cla(.+?)lbum_report__second_line"><span class="album_report__date">(.+?)</span>'
    ).findall(link)
...
def play_album(name, url, iconimage, mix, clear):
    ...
    if GOLDEN_PATH:
        url = (
            ...
            .replace("musicmp3", "www.goldenmp3")
            .replace("artist_", "/")
            .replace("__album_", "/")
            .replace(".html", "")
        )
    ...
    if "musicmp3" in origurl:
        ...
        "goldenmp3" in origurl
    ):  # track,id,songurl,meta, d1,album,artist,songname,dur,artist1
    ...
    if mode != 6 and mix != "mix" and mix != "queue":
        if dialog.yesno(
            "MP3 Streams", "Browse songs or play full album?", "Play Now", "Browse"
        ):
            browse = True
    match = []
    link = GET_url(url)  # .decode('utf-8')
    ...
    if "musicmp3" in origurl:
        ...
        for row in rows:
            ...
            rel = play_btn["rel"][0] if play_btn else ""  # replaces your alt fallback
            ...
            el = row.find("div", class_="jp-seek-bar")
            dur = el["data-time"] if el is not None else ""
    ...
    if browse:
        for track, id, songurl, meta, d1, album, artist, songname, dur in match:
            ...
            if GOTHAM_FIX:
                ...
                url = (
                    "https://listen.musicmp3.ru/" + id
                )  #'http://files.musicmp3.ru/lofi/' + id #find_url(trn).strip() + id
    ...
    for track, id, songurl, meta, d1, album, artist, songname, dur in match:
        ...
        if GOTHAM_FIX:
            try:
                alturl = "http://www.myfreemp3.eu/music/%s+%s" % (
                    artist.replace("&amp; ", "")
                    .replace("& ", "")
                    .replace(" and ", " "),
                    songname.replace("&amp; ", "")
            ...
            url = "https://listen.musicmp3.ru/" + str(
                id
            )  #'http://files.musicmp3.ru/lofi/' + id #find_url(trn).strip() + id
        songname = songname.replace("&amp;", "and")
        if "musicmp3" in origurl:
...
def download_song(url, name, songname, artist, album, iconimage):
    ...
    Uses playerMP3.STREAM_HEADERS — the canonical header definition — so
    that Host, User-Agent, Accept, and Referer are always correct and
    consistent with every other request made to listen.musicmp3.ru.
    """
    ...
    try:
        response = requests.get(
            url,
            headers=playerMP3.STREAM_HEADERS,  # single source of truth for CDN headers
            stream=True,
            verify=True,
            timeout=30,
        )
        ...
        xbmc.log(
            "download_song: HTTP %d for %s — %s" % (e.response.status_code, url, e),
            xbmc.LOGERROR,
        )
        return
    ...
    if os.path.isfile(FAV_SONG):
        ...
        for list in search_list:
            if list != "":
                ...
                url1 = splitdata[3].replace(
                    "listen.musicmp3.ru", "files.musicmp3.ru/lofi"
                )
        ...
        for list in search_list:
            if list != "":
                ...
                if os.path.exists(icon_path):
                    ...
                    iconimage = iconart
                addDir(
                    title.replace("&amp;", "&").upper(), url, 22, iconimage, "artists"
                )
...
def add_favourite(name, url, dir, text):
    ...
    if "artist" in dir:
        ...
        try:
            ...
            xbmc.log(
                "977 artist = {0}\nartist_url = {1}".format(artist, artist_url),
                xbmc.LOGINFO,
            )
        except:
            pass
...
def add_favourite_song(name, url, dir, text):
    ...
    add_to_list(url, dir, False)
    notification(
        songname.upper(), "[COLOR lime]" + text + "[/COLOR]", "5000", iconimage
    )
...
def notification(title, message, ms, nart):
    xbmc.executebuiltin(
        "XBMC.notification(" + title + "," + message + "," + ms + "," + nart + ")"
    )
...
def regex_from_to(text, from_string, to_string, excluding=True):
    if excluding:
        r = re.search("(?i)" + from_string + r"([\S\s]+?)" + to_string, text).group(1)
    else:
        r = re.search(
            "(?i)(" + from_string + r"[\S\s]+?" + to_string + ")", text
        ).group(1)
    return r
    ...
    if type == "artists":
        if find_list(list, FAV_ARTIST) < 0:
            ...
            contextMenuItems.append(
                (
                    "[COLOR lime]Add to Favourite Artists[/COLOR]",
                    "RunPlugin(%s?name=%s&url=%s&mode=61)"
                    % (sys.argv[0], name, str(list)),
                )
            )
        else:
            ...
            contextMenuItems.append(
                (
                    "[COLOR orange]Remove from Favourite Artists[/COLOR]",
                    "RunPlugin(%s?name=%s&url=%s&mode=62)"
                    % (sys.argv[0], name, str(list)),
                )
            )
    if "album" in type:
        download_album = "%s?name=%s&url=%s&iconimage=%s&mode=202" % (
            sys.argv[0],
        ...
        xbmc.log(
            "1230 sys.argv[0] = {0}\nurllib.quote(name) = {1}\nurl = {2}\niconimage = {3}".format(
                sys.argv[0], urllib.parse.quote(name), url, iconimage
            ),
            xbmc.LOGINFO,
        )
        contextMenuItems.append(
            ("[COLOR cyan]Download Album[/COLOR]", "RunPlugin(%s)" % download_album)
        )
        if os.path.exists(download_lock):
            contextMenuItems.append(
                (
                    "Clear Download Lock",
                    "RunPlugin(%s?name=%s&url=%s&iconimage=%s&mode=333)"
                    % (sys.argv[0], urllib.parse.quote(name), url, iconimage),
                )
            )
        if QUEUE_ALBUMS:
            ...
            contextMenuItems.append(
                ("[COLOR cyan]Play/Browse Album[/COLOR]", "RunPlugin(%s)" % play_music)
            )
        else:
        ...
        if not "qq" in type1:
            ...
            contextMenuItems.append(
                (
                    "[COLOR lime]Add to Favourite Albums[/COLOR]",
                    "RunPlugin(%s?name=%s&url=%s&mode=64)"
                    % (sys.argv[0], name.replace(",", ""), str(list)),
                )
            )
        else:
            suffix = " [COLOR lime]+[/COLOR]"
            contextMenuItems.append(
                (
                    "[COLOR orange]Remove from Favourite Albums[/COLOR]",
                    "RunPlugin(%s?name=%s&url=%s&mode=65)"
                    % (sys.argv[0], name.replace(",", ""), str(list)),
                )
            )
...
def addDirAudio(name, url, mode, iconimage, songname, artist, album, dur, type):
    ...
    if os.path.exists(download_lock):
        contextMenuItems.append(
            (
                "Clear Download Lock",
                "RunPlugin(%s?name=%s&url=%s&iconimage=%s&songname=%s&artist=%s&album=%s&mode=333)"
                % (sys.argv[0], songname, url, iconimage, name, artist, album),
            )
        )
    download_song = (
        "%s?name=%s&url=%s&iconimage=%s&songname=%s&artist=%s&album=%s&mode=201"
        % (sys.argv[0], songname, url, iconimage, name, artist, album)
    )
    contextMenuItems.append(
        ("[COLOR cyan]Download Song[/COLOR]", "RunPlugin(%s)" % download_song)
    )
    if QUEUE_SONGS:
        play_song = (
            "%s?name=%s&url=%s&iconimage=%s&songname=%s&artist=%s&album=%s&dur=%s&mode=18"
            % (
                sys.argv[0],
                urllib.parse.quote(songname),
                url,
                iconimage,
                songname,
                artist,
        ...
        contextMenuItems.append(
            ("[COLOR cyan]Play Song[/COLOR]", "RunPlugin(%s)" % play_song)
        )
    else:
        queue_song = (
            "%s?name=%s&url=%s&iconimage=%s&songname=%s&artist=%s&album=%s&dur=%s&mode=11"
            % (
                sys.argv[0],
                urllib.parse.quote(songname),
                url,
        ...
        contextMenuItems.append(
            ("[COLOR cyan]Queue Song[/COLOR]", "RunPlugin(%s)" % queue_song)
        )
    if type != "favsong":
        suffix = ""
        contextMenuItems.append(
            (
                "[COLOR lime]Add to Favourite Songs[/COLOR]",
                "RunPlugin(%s?name=%s&url=%s&mode=67)"
                % (sys.argv[0], name.replace(",", ""), str(list)),
            )
        )
    else:
        ...
        contextMenuItems.append(
            (
                "[COLOR orange]Remove from Favourite Songs[/COLOR]",
                "RunPlugin(%s?name=%s&url=%s&mode=68)"
                % (sys.argv[0], name.replace(",", ""), str(list)),
            )
        )
    liz = xbmcgui.ListItem(name + suffix)
